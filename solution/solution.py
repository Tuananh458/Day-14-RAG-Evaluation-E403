...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            relevancy.append(1 if coverage >= relevance_threshold else 0)

        num_relevant = sum(relevancy)
        if num_relevant == 0:
            return 0.0

        ap_sum = 0.0
        relevant_so_far = 0
        for idx, rel in enumerate(relevancy):
            k = idx + 1
            if rel == 1:
                relevant_so_far += 1
    ...
    def run_full_eval(
        ...
        if retrieved_contexts:
            precision_score = self.evaluate_context_precision(retrieved_contexts, expected)
            recall_score = self.evaluate_context_recall(retrieved_contexts, expected)
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric: {rubric}\n"
            f"Please score this answer based on the rubric criteria. Return a JSON object where keys are the criteria name and values are floats from 0 to 1."
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(scores_batch) >= 2:
            mid = len(scores_batch) // 2
            first_half = scores_batch[:mid]
            second_half = scores_batch[mid:]

            first_scores = [v for item in first_half for v in item.get("scores", {}).values()]
            second_scores = [v for item in second_half for v in item.get("scores", {}).values()]
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_f = sum(r.faithfulness for r in new_results) / new_len if new_len > 0 else 0.0
        new_r = sum(r.relevance for r in new_results) / new_len if new_len > 0 else 0.0
        new_c = sum(r.completeness for r in new_results) / new_len if new_len > 0 else 0.0

        base_f = sum(r.faithfulness for r in baseline_results) / base_len if base_len > 0 else 0.0
        base_r = sum(r.relevance for r in baseline_results) / base_len if base_len > 0 else 0.0
        base_c = sum(r.completeness for r in baseline_results) / base_len if base_len > 0 else 0.0
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        min_val = min(f, r, c)
        count_min = sum(1 for v in (f, r, c) if v == min_val)
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, f in enumerate(failures):
            ...
            if idx < len(suggestions):
                ...
                fix = suggestions[idx % len(suggestions)]
            else:
    ...
    def generate_improvement_suggestions(
        ...
        if cats.get("irrelevant", 0) > 0 or cats.get("off_topic", 0) > 0:
            suggestions.append("Add few-shot examples showing complete answers to improve completeness and relevance")

        default_suggestions = [
            "Review retriever recall and adjust top-k parameter to fetch more relevant context",
            "Refine agent prompt instructions to prevent out-of-bounds answers",
            "Implement input guardrails to reject out-of-scope queries early"
        ]
